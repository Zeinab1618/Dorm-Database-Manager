    ...
    with st.expander("➕ Add Student"):
        ...
        if st.button("Add Student"):
            if free_slots <= 0:
                ...
                cursor.execute("INSERT INTO Penalty (student_id) VALUES (%s)", (sid,))
        ...
        if student:
            ...
            if meal:
                ...
                if st.button("Update Meal"):
                    cursor.execute("REPLACE INTO Meals (student_id, meal_type, weekday) VALUES (%s, %s, %s)", (search_id, new_meal, new_day))
                    conn.commit()
...
# ---------------------- PENALTY TABLE ----------------------
    ...
    if st.button("Update Penalty"):
        cursor.execute("UPDATE Penalty SET total_points = %s WHERE student_id = %s", (points, pid))
        conn.commit()
